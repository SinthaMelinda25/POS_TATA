        ...
        <table class="table table-bordered mb-0 keranjang-table">
         ...
         <div class="card-footer">
            ...
            <form method="POST" 
                ...
                <select name="payment_method" id="metodePembayaran" class="form-select mb-2 payment-select-pos">
                    <option value="">Pilih Pembayaran</option>
                    <option value="CASH" {{ old('payment_method') === 'CASH' ? 'selected' : '' }}>Cash</option>
                    <option value="QRIS" {{ old('payment_method') === 'QRIS' ? 'selected' : '' }}>QRIS</option>
                </select>

                <div id="bayarCash" class="mb-2" style="display:none;">
                    <input type="number" name="uang_masuk" id="uangMasuk" class="form-control mb-1"
                           placeholder="Uang Masuk" min="0" step="500">
                    <small id="pesanKurang" class="text-danger" style="display:none;">Uang tidak cukup</small>
                    <div class="d-flex justify-content-between mt-1">
                        <span>Kembalian:</span>
                        <strong id="uangKembalianText">Rp 0</strong>
                    </div>
                </div>

                <div id="bayarQris" class="mb-2 text-center" style="display:none;">
                    <p class="mb-2">Scan kode QRIS berikut untuk membayar Rp {{ number_format($sale->total_pembayaran) }}:</p>
                    <div id="qrcodeBox" class="d-flex justify-content-center"></div>
                </div>

                <button class="btn btn-success btn-checkout-pos w-100 {{ $sale->status === 'COMPLETED' ? 'disabled' : '' }}" id="btnCheckoutPos">
                    Checkout
                </button>
             </form>
            @can('delete', $sale)
...
{{-- Library QRCode.js (davidshimjs/qrcodejs, MIT license) di-inline langsung di sini,
     jadi tidak perlu file terpisah di public/js/qrcode.min.js --}}
<script>
/**
 * @fileoverview
...
<script>
    ...
    // Pakai id transaksi asli dari database (bukan Date.now()), biar konsisten dengan record penjualan
    const kodeTransaksi   = 'TRX-{{ str_pad($sale->id, 6, "0", STR_PAD_LEFT) }}';

    document.addEventListener('DOMContentLoaded', function () {
        const selectMetode    = document.getElementById('metodePembayaran');
        const bayarCash       = document.getElementById('bayarCash');
        const bayarQris       = document.getElementById('bayarQris');
        const inputUangMasuk  = document.getElementById('uangMasuk');
        const kembalianText   = document.getElementById('uangKembalianText');
        const pesanKurang     = document.getElementById('pesanKurang');
        const btnCheckout     = document.getElementById('btnCheckoutPos');
        const qrcodeBox       = document.getElementById('qrcodeBox');

        let qrGenerated = false;

        function generateQrJikaBelum() {
            if (qrGenerated) return;

            // Jaga-jaga kalau file qrcode.min.js belum ter-load / belum ada di public/js
            if (typeof QRCode === 'undefined') {
                qrcodeBox.innerHTML =
                    '<p class="text-danger small mb-0">Gagal memuat QR Code. Pastikan file qrcode.min.js ada di public/js.</p>';
                return;
            }

            qrcodeBox.innerHTML = '';
            new QRCode(qrcodeBox, {
                text: 'Pembayaran QRIS | ' + kodeTransaksi + ' | Total: Rp' + totalPembayaran,
                width: 200,
                height: 200
            });
            qrGenerated = true;
        ...
        function tampilkanSesuaiMetode() {
            if (selectMetode.value === 'CASH') {
                bayarCash.style.display = 'block';
                bayarQris.style.display = 'none';
                btnCheckout.disabled = false;
            } else if (selectMetode.value === 'QRIS') {
                bayarCash.style.display = 'none';
                bayarQris.style.display = 'block';
                btnCheckout.disabled = false;
                generateQrJikaBelum();
            } else {
                bayarCash.style.display = 'none';
        ...
        // menangani kasus dropdown sudah dalam posisi QRIS tanpa event "change" ter-trigger
        tampilkanSesuaiMetode();

        // Jalankan lagi setiap kali kasir ganti pilihan
        selectMetode.addEventListener('change', tampilkanSesuaiMetode);

        if (inputUangMasuk) {
            inputUangMasuk.addEventListener('input', function () {
                const masuk = parseFloat(inputUangMasuk.value) || 0;
                const kembalian = masuk - totalPembayaran;

                if (masuk < totalPembayaran) {
